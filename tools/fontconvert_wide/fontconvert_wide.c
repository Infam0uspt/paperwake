...
// Fork of Adafruit-GFX-Library's fontconvert, for glyphs too large
// for the classic GFXglyph format: that struct's xOffset/yOffset are
// int8_t (range -128..127), width/height are uint8_t (max 255), and
// bitmapOffset is uint16_t (max 65535 bytes of bitmap data) — all of
// which silently wrap around for big fonts (~130px+ tall glyphs)
// without any warning. WideGlyph/WideFont widen those fields so large
// fonts (e.g. a 210px clock digit) encode correctly. See
// include/WideFont.h for the matching firmware-side struct + the
// custom renderer that reads it (display.setFont()/print() only
// understands the classic GFXglyph format).
typedef struct {
...
int main(int argc, char *argv[]) {
  ...
  if ((!(fontName = malloc(strlen(ptr) + 20))) ||
      (!(table = (WideGlyph *)malloc((last - first + 1) * sizeof(WideGlyph))))) {
  ...
  printf("const WideGlyph %sGlyphs[] PROGMEM = {\n", fontName);
  for (i = first, j = 0; i <= last; i++, j++) {
  ...
  printf("const WideFont %s PROGMEM = {\n", fontName);
  printf("  (uint8_t  *)%sBitmaps,\n", fontName);
  printf("  (WideGlyph *)%sGlyphs,\n", fontName);
